...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO Aduana (ID_Ad, Estado_Ad, Ubicacion_Ad, Telefono_Ad) VALUES (?, ?, ?, ?)";

    if($stmt = $conn->prepare($sql)){
        $stmt->bind_param("isss", $numero, $estado, $ubicacion, $telefono);
        
        if($stmt->execute()){
            echo "<script>alert('Aduana añadida exitosamente');</script>";
        } else {
            echo "<script>alert('Error al añadir la aduana');</script>";
        }
...
<div class="container mt-5">
    <h2 class="text-center">Alta de Aduana</h2>
    <form method="post" action="altaAduana.php">
        <div class="form-group">
            <label for="numero">Número de Aduana</label>
            <input type="number" class="form-control" id="numero" name="numero" placeholder="Número de aduana" required>
        </div>
        <div class="form-group">
            <label for="estado">Estado</label>
            <input type="text" class="form-control" id="estado" name="estado" placeholder="Estado donde se ubica la aduana" required>
        </div>
        <div class="form-group">
            <label for="ubicacion">Ubicación</label>
            <input type="text" class="form-control" id="ubicacion" name="ubicacion" placeholder="Ubicación exacta de aduana" required>
        </div>
        <div class="form-group">
            <label for="telefono">Teléfono</label>
            <input type="text" class="form-control" id="telefono" name="telefono" pattern="\d{10}" placeholder="Teléfono de la aduana" required>
