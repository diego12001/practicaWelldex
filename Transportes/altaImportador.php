...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO Importador (Empresa_Imp, Operador_Imp, Telefono_Imp, Correo_Imp, Pais_Imp, Ubicacion) VALUES (?, ?, ?, ?, ?, ?)";

    if($stmt = $conn->prepare($sql)){
        $stmt->bind_param("ssssss", $empresa, $operador, $telefono, $correo, $pais, $ubicacion);
        
        if($stmt->execute()){
            echo "<script>alert('Importador añadido exitosamente');</script>";
        } else {
            echo "<script>alert('Error al añadir el importador');</script>";
        }
...
<head>
    ...
    <title>Alta de Importador</title>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <div class="container mt-5">
        <h2 class="text-center">Alta de Importador</h2>
        <form method="post" action="altaImportador.php">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="operador" name="operador" placeholder="Nombre del operador" required>
            </div>
            <div class="form-group">
                <label for="telefono">Teléfono</label>
                <input type="text" class="form-control" id="telefono" name="telefono" pattern="\d{10}" placeholder="Teléfono del importador" required>
                <small class="form-text text-muted">Debe ser un número de 10 dígitos.</small>
            </div>
            <div class="form-group">
                <label for="correo">Correo Electrónico</label>
                <input type="email" class="form-control" id="correo" name="correo" placeholder="Correo electrónico del importador" required>
            </div>
            <div class="form-group">
                <label for="pais">País</label>
                <input type="text" class="form-control" id="pais" name="pais" placeholder="País del importador" required>
            </div>
            <div class="form-group">
                <label for="ubicacion">Ubicación</label>
                <input type="text" class="form-control" id="ubicacion" name="ubicacion" placeholder="Ubicación del importador" required>
            </div>
