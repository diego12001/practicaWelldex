...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO Transportista (Empresa_Transp, Telefono_Transp, Correo_Transp, Estado_Transp, Ubicacion_Transp) VALUES (?, ?, ?, ?, ?)";

    if($stmt = $conn->prepare($sql)){
        $stmt->bind_param("sssss", $empresa, $telefono, $correo, $estado, $ubicacion);
        
        if($stmt->execute()){
            echo "<script>alert('Transportista añadido exitosamente');</script>";
        } else {
            echo "<script>alert('Error al añadir el transportista');</script>";
        }
...
<div class="container mt-5">
    <h2 class="text-center">Alta de Transportista</h2>
    <form method="post" action="altaTransportista.php">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="telefono" name="telefono"             pattern="\d{10}" placeholder="Teléfono de la transportista" required>
            <small class="form-text text-muted">Debe ser un número de 10 dígitos.</small>
        </div>
        <div class="form-group">
            <label for="correo">Correo Electrónico</label>
            <input type="email" class="form-control" id="correo" name="correo" placeholder="Correo de la transportista" required>
        </div>
        <div class="form-group">
            <label for="estado">Estado</label>
            <input type="text" class="form-control" id="estado" name="estado" placeholder="Estado de la transportista" required>
        </div>
        <div class="form-group">
            <label for="ubicacion">Ubicación</label>
            <input type="text" class="form-control" id="ubicacion" name="ubicacion" placeholder="Ubicación de la transportista" required>
        </div>
