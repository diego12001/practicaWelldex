<?php
session_start();
if(!isset($_SESSION['login_user'])){
    header("location: index.html");
    exit;
}

include('bd.php');

$idSolicitud = $_GET['id'];

$sql = "SELECT 
            S.ID_Solicitud, 
            S.Referencia, 
            C.Nombre_Cli, 
            M.Nombre_Merc AS Nombre_Mercancia, 
            TM.TipoM AS Tipo_Mercancia, 
            I.Empresa_Imp AS Importador, 
            T.Empresa_Transp AS Transportista, 
            E.Nombre_Estado AS Estado,
            (SELECT GROUP_CONCAT(CONCAT(CO.Serie_Contenedor, ' - ', TC.Descripcion_Cont) SEPARATOR ', ') 
             FROM ContenedoresSoli CO 
             JOIN TipoContenedor TC ON CO.ID_TipoC = TC.ID_TipoC 
             WHERE CO.ID_Solicitud = S.ID_Solicitud) AS Contenedores,
            (SELECT GROUP_CONCAT(TR.Matricula SEPARATOR ', ') 
             FROM Respuesta R 
             JOIN Transporte TR ON R.Matricula = TR.Matricula 
             WHERE R.ID_Solicitud = S.ID_Solicitud) AS Vehiculos
        FROM Solicitud S
        JOIN Operacion O ON S.Referencia = O.Referencia
        JOIN Cliente C ON O.ID_Cli = C.ID_Cli
        JOIN Mercancia M ON O.ID_Merc = M.ID_Merc
        JOIN TipoMercancia TM ON M.ID_TipoM = TM.ID_TipoM
        JOIN Importador I ON O.ID_Imp = I.ID_Imp
        JOIN Transportista T ON S.ID_Transp = T.ID_Transp
        JOIN Estado E ON S.ID_Estado = E.ID_Estado
        WHERE S.ID_Solicitud = ?";

$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $idSolicitud);
$stmt->execute();
$result = $stmt->get_result();

$solicitud = $result->fetch_assoc();

$conn->close();
?>

<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Información de la Solicitud</title>
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
    <link href="css/informacion.css" rel="stylesheet">
</head>
<body>

<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <a class="navbar-brand" href="welcome.php">Sistema de Transportistas</a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
        <ul class="navbar-nav mr-auto">
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" href="#" id="altasDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    Altas
                </a>
                <div class="dropdown-menu" aria-labelledby="altasDropdown">
                    <a class="dropdown-item" href="altaTransporte.php">Transportes</a>
                    <a class="dropdown-item" href="altaOperacion.php">Operacion</a>
                    <a class="dropdown-item" href="altaMercancia.php">Mercancia</a>
                    <a class="dropdown-item" href="altaCliente.php">Cliente</a>
                    <a class="dropdown-item" href="altaAduana.php">Aduana</a>
                    <a class="dropdown-item" href="altaImportador.php">Importador</a>
                    <a class="dropdown-item" href="altaTransportista.php">Transportista</a>
                </div>
            </li>
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" href="#" id="solicitudesDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    Solicitudes
                </a>
                <div class="dropdown-menu" aria-labelledby="solicitudesDropdown">
                    <a class="dropdown-item" href="verSolicitudes.php">Ver Solicitudes</a>
                    <a class="dropdown-item" href="crearSolicitud.php">Crear Solicitud</a>
                    <a class="dropdown-item" href="asignarTransporte.php">Asignar Transportes</a>
                    <a class="dropdown-item" href="cambiarEstado.php">Cambiar Estado de Solicitudes</a>
                </div>
            </li>
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" href="#" id="informacionDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    Información
                </a>
                <div class="dropdown-menu" aria-labelledby="informacionDropdown">
                    <a class="dropdown-item" href="verTransporte.php">Ver Transportes</a>
                    <a class="dropdown-item" href="verMercancia.php">Ver Mercancías</a>
                    <a class="dropdown-item" href="verCliente.php">Ver Clientes</a>
                    <a class="dropdown-item" href="verTransportista.php">Ver Transportistas</a>
                    <a class="dropdown-item" href="verImportador.php">Ver Importadores</a>
                    <a class="dropdown-item" href="verAduana.php">Ver Aduana</a>
                </div>
            </li>
        </ul>
        <ul class="navbar-nav ml-auto">
            <li class="nav-item">
                <a class="nav-link" href="logout.php">Cerrar Sesión</a>
            </li>
        </ul>
    </div>
</nav>

<div class="container mt-5">
    <div class="report-header">
        <h2>Información de la Solicitud</h2>
    </div>
    <div class="report-container">
        <div class="report-section">
            <div class="section-title">Información General</div>
            <div class="report-item"><span>Referencia:</span> <?php echo $solicitud['Referencia']; ?></div>
            <div class="report-item"><span>Cliente:</span> <?php echo $solicitud['Nombre_Cli']; ?></div>
            <div class="report-item"><span>Nombre Mercancía:</span> <?php echo $solicitud['Nombre_Mercancia']; ?></div>
            <div class="report-item"><span>Tipo Mercancía:</span> <?php echo $solicitud['Tipo_Mercancia']; ?></div>
            <div class="report-item"><span>Importador:</span> <?php echo $solicitud['Importador']; ?></div>
        </div>
        <div class="report-section">
            <div class="section-title">Transporte</div>
            <div class="report-item"><span>Transportista:</span> <?php echo $solicitud['Transportista']; ?></div>
            <div class="report-item"><span>Contenedores:</span> <?php echo $solicitud['Contenedores'] ? $solicitud['Contenedores'] : 'No Aplica'; ?></div>
            <div class="report-item"><span>Vehículos:</span> <?php echo $solicitud['Vehiculos']; ?></div>
        </div>
        <div class="report-section">
            <div class="section-title">Estado de la Solicitud</div>
            <div class="report-item"><span>Estado:</span> <?php echo $solicitud['Estado']; ?></div>
        </div>
        <div class="back-button">
            <a href="verSolicitudes.php" class="btn btn-primary">Regresar a Solicitudes</a>
        </div>
    </div>
</div>

<script src="https://code.jquery.com/jquery-3.5.1.js"></script>
<script src="https://cdn.datatables.net/1.10.21/js/jquery.dataTables.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.5.4/dist/umd/popper.min.js"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
</body>
</html>
